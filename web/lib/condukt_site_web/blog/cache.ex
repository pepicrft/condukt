defmodule ConduktSiteWeb.Blog.Cache do
  @moduledoc """
  Caches rendered blog posts by source path and modification time.

  Post edits are picked up without restarting the development server, while
  production requests avoid re-rendering the same Markdown on every request.
  """
  ...
  def handle_call({:render, file, modified_at}, _from, state) do
    page =
      case :ets.lookup(@table, file) do
        ...
        _ ->
          page = file |> File.read!() |> Markdown.render(file)
